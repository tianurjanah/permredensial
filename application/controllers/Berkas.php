	...
	public function tambah_ijazah()
	{
		...
		if (!empty($_FILES['berkaslampiran']['name'])) {
			if (!$this->upload->do_upload('berkaslampiran')) {
				...
				$datalampiran = array('berkaslampiran' => $this->upload->data());
				$nama_file_lampiran = $datalampiran['berkaslampiran']['file_name'];
		...
		// Proses upload dan perubahan file scan KTP
		if (!empty($_FILES['berkastranskip']['name'])) {
			if (!$this->upload->do_upload('berkastranskip')) {
		...
		$where = array('id_user' => $this->session->userdata('login_session')['id_user']);
		$existingData = $this->berkas_model->detail_data($where, 'ijazah');
		if ($existingData) {
			$this->berkas_model->ubah_data($where, $data, 'ijazah');
			$pesan = 'Data berhasil diubah!';
		} else {
			$this->berkas_model->tambah_data($data, 'ijazah');
			$pesan = 'Data berhasil ditambahkan!';
		}
